...
start: _work/clear-kvm.img _work/kube-clear-kvm _work/start-clear-kvm _work/ssh-clear-kvm test/setup-ca-kubernetes.sh _work/.setupcfssl-stamp
	...
	if [ -e deploy/kubernetes-$$(cat _work/clear-kvm-kubernetes.version)/csinodeinfo-crd.yaml ]; then \
	   	if ! _work/ssh-clear-kvm kubectl get crd/csinodeinfos.csi.storage.k8s.io  >/dev/null 2>&1; then \
			_work/ssh-clear-kvm kubectl create -f - <deploy/kubernetes-$$(cat _work/clear-kvm-kubernetes.version)/csinodeinfo-crd.yaml; \
		fi \
	fi
