...
IMAGE_BUILD_ARGS=--build-arg GO_VERSION=1.12.4 --build-arg NDCTL_VERSION=64.1 --build-arg NDCTL_CONFIGFLAGS='--libdir=/usr/lib --disable-docs --without-systemd --without-bash' \
--build-arg NDCTL_BUILD_DEPS='autoconf m4 pkg-config libtool libuuid-devel kmod-devel libudev-devel util-linux json-c-devel keyutils-libs-devel linux-headers-virtual wget tar file'
